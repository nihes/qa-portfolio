...
- **One npm project per suite.** Each folder has its own `package.json` and
  ...
  independently versioned, and is what lets CI run all thirteen automated jobs
...
`workflow_dispatch`. It is a GitHub Actions workflow with **thirteen independent
jobs** — one per runnable automated suite (each scoped to its own folder via
`working-directory` so it installs and runs in isolation), plus a `typecheck`
static-analysis gate:

`api-newman` · `api-mocha` · `api-graphql` · `playwright` · `cypress` ·
`cucumber` · `selenium` · `mobile-web` · `accessibility` · `email` ·
`security` · `test-data` · `typecheck`
