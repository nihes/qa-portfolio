...
different tool ecosystems, BDD, two different styles of API testing, mobile
(emulated and real-device), and the non-functional disciplines (accessibility,
performance, visual regression, email) that separates the "confirm the happy
path with a UI script" QA from a senior generalist who can pick the right
tool and testing layer for a given risk.
...
├── docs/                        # this folder — matrix, contributing, architecture
...
Each of the 13 discipline folders is a **fully independent npm project**
...
| **[dummyjson.com](https://dummyjson.com/)** | `api-mocha/`, `performance/` | A public JSON API with JWT auth and full CRUD endpoints, well suited to code-based contract testing (ajv/JSON-Schema) and to load testing — it doesn't persist writes, which the suites document and test around explicitly (asserting the request/response *contract*, not stored state) |
...
**nine independent jobs**, each scoped to one suite's folder:
...
- **Breadth across tools and layers.** The same core e-commerce journeys
  ...
  UI, API (two styles), BDD, mobile (two styles), and three non-functional
  disciplines, mapped explicitly to test levels (component/integration/
  system/E2E) in `manual-testing/test-strategy.md`.
